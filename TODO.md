...
- One dashboard, when we show all of the users morpho vault positions (inlcluding non muscadine vaults, have the ability to click on the position and lead to the the /vault/v2 page for the vault. Than on the page for withdraw/deposit lead to a model that says on this frontend you cannot interact with this vault, if you wish to depsoit/withdraw from this vault you must do it through the morpho front end, than have a continue or cancel button with continue being linkted to the morpho vault front end website. 
