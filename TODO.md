...
- **Vault v2 force exit when withdraw > liquidity:** If requested withdraw/redeem exceeds Morpho GraphQL `liquidity` (instant idle + liquidity adapter), offer or auto-route via `forceDeallocate` then normal `withdraw`/`redeem` in `transactionUtilsV2.ts` (or Morpho SDK `forceWithdraw`/`forceRedeem` if we adopt the SDK). Cap MAX to instant liquidity until implemented; surface clear UX when full exit needs force path (curator penalty up to ~2%). Users can force-exit via [Morpho app](https://app.morpho.org) in the meantime. Ref: [Morpho force deallocate](https://morpho-org-vault-v2.mintlify.app/operations/force-deallocate).
