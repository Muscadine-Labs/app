...
- **Deposit gates:** config allowlist first, then `canSendAssets` on the send-assets gate (`useUnderlyingDepositAccess`). A failed read keeps the config result. Sync the allowlist from curator; run curator `npm run gates:verify` after allowlist edits. See `CLAUDE.md` § Configuration. Kind filter (All / Underlying / Wrappers) is cached per wallet in `localStorage`.
