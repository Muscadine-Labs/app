...
- Registry: `src/lib/vaults.ts` — fields include `strategy` (`prime` | `frontier`), `kind` (`wrapper` | `underlying`), `vaultSymbol` (e.g. `wmpUSDC`, `mpUSDC`, `wmpcbBTC`). Explorer uses gate-driven visibility (`useVaultDepositGates`, `vault-access.ts`). Confirm Morpho dead shares (`0x…dEaD`) **before listing**; do not add a runtime RPC check — curated vaults already have them.
- **Deposit gates:** no allowlist constants. `useVaultDepositGates` reads each vault's own `canSendAssets` / `canReceiveShares` on chain as soon as the app and wallet load (true when the vault has no gate). Underlying deposits open only on a successful yes; a vault with no gates is open to everyone. Wrappers get the same wallet checks, plus their liquidity adapter must pass the child vault's checks; they disable only on a successful no. Withdraw stays open. Deposits re-check the gates on chain before any approval (`readVaultDepositBlocker`; only a successful no blocks). The Bundler3 ETH option needs `canSendAssets(GeneralAdapter1)`. See `CLAUDE.md` § Deposit gates. Settings shows a “Wrapper vaults” switch (`role="switch"`, blue with the knob right when wrappers are on) when wrappers can still take deposits and the wallet either can deposit into every underlying vault (opens on Underlying) or holds underlying shares (opens on Wrappers; the Underlying list is view-only, with deposits blocked and withdrawals open). If every wrapper is blocked, there is no switch. Other wallets stay on Wrappers, with any underlying they can deposit into or hold added. The settings choice is not cached.
...
- **No developer / over-balance bypass mode** — `VaultVersionContext` removed. Explorer filters default to All for strategy/asset; transact blocks amounts over balance. `/vaults` defaults to **Wrappers** for the public. Wallets that can deposit into every underlying vault default to **Underlying** (everyone, once all underlying gates are removed).

## Dashboard & positions

- **Wallet strip:** Total / Wallet (liquid) / Vaults USD (`WalletOverview`).
- **Your Vaults** (dashboard): **v2** positions only (registry + external Morpho v2 vaults). Which vaults it lists does not follow the explorer kind switch. **Kind labels** (**wrapper** / **underlying**) only show where kinds mix: a vault outside the current list kind, plus the other side of its pair (`kindMarkAddresses` in `VaultKindContext`). A whitelisted wallet holding only underlyings sees no labels.
